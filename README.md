# 🤖 FTC Dubai 2025

<p align="center">
  <img width="1280" height="854" alt="photo_2025-07-01_19-33-59" src="https://github.com/user-attachments/assets/6b90e42a-206e-47a2-ba6a-6b41aeba60ba" />

</p>

<img width="1280" height="854" alt="photo_2025-07-01_19-33-59" src="https://github.com/user-attachments/assets/463b584d-fffb-4dfb-9833-4c94ef6633ee" />




---

# 🏆 Competition Overview

In 2025, our team traveled to **Dubai** to compete in a **FIRST Tech Challenge (FTC)** event.

After preparing together as a team, we participated in the competition with our robot and competed against other teams in a challenging FTC environment.

Our team successfully reached the **Finalist Alliance** and finished the competition in **2nd place**.

We also received the **Control Award** for our robot's control system.

This event was an important milestone for our team and gave us valuable experience in competitive robotics, engineering, programming, robot operation, teamwork, and match strategy.

---

## 📍 Event Information

|                 |                            |
| --------------- | -------------------------- |
| 🏆 Event        | FTC Dubai 2025             |
| 🤖 Category     | FIRST Tech Challenge (FTC) |
| 📍 Location     | Dubai, UAE                 |
| 📅 Year         | 2025                       |
| 🥈 Final Result | **2nd Place**              |
| 🏅 Alliance     | **Finalist Alliance**      |
| 🎖️ Award       | **Control Award**          |
| 👥 Team Size    | 4 Members                  |

---

# 🥈 Competition Achievement

## Finalist Alliance

<p align="center">
 <img width="1543" height="1295" alt="Снимок экрана 2026-09-03 151731_LE_upscale_prime" src="https://github.com/user-attachments/assets/caa2e4e2-9bcb-4a60-83a8-01f377dcd3ba" />

</p>

Our team became part of the **Finalist Alliance** and finished the competition in **2nd place**.

Reaching the finals required strong teamwork, consistent robot performance, effective communication, and good match strategy.

---

# 🎖️ Control Award

<p align="center">
  <img width="895" height="533" alt="photo_2026-09-03_15-02-50" src="https://github.com/user-attachments/assets/f55429c2-6f72-4abb-86d3-5515898213ee" />

</p>

Our team also received the **Control Award**.

The award recognized the strength of our robot's **control system** and the way we used control and software technologies to operate the robot effectively.

This achievement was especially meaningful because it demonstrated the connection between our robot's engineering, programming, and competition performance.

---

# 👥 Our Team

Our team consisted of four members with different technical and competition responsibilities.

<p align="center">
  <img src="YOUR_TEAM_PHOTO.jpg" width="750">
</p>

---

## 📐 Shoxruxmirzo Xafizov

### CAD Designer • Driver

<p align="center">
  <img src="YOUR_SHOXRUXMIRZO_PHOTO.jpg" width="300">
</p>

I worked as a **CAD Designer** and **Driver**.

### Responsibilities

* CAD design
* Mechanical design
* Robot component development
* Mechanism design
* Supporting robot assembly
* Testing mechanical systems
* Competition robot operation

### Competition Role

I was one of the two drivers.

I controlled the robot's **manipulator mechanisms**, while the second driver controlled the robot's movement.

---

## 🔧 Nazarov Asadbek

### Engineer • Driver

<p align="center">
  <img src="YOUR_ASADBEK_PHOTO.jpg" width="300">
</p>

Asadbek worked as an **Engineer** and **Driver**.

### Responsibilities

* Engineering
* Robot assembly
* Mechanical testing
* Troubleshooting
* Technical development
* Robot reliability

### Competition Role

Asadbek was the second driver and controlled the **movement and driving of the robot**.

---

## 💻 Narziyev Akbarshox

### Coder • Human Player

<p align="center">
  <img src="YOUR_AKBARSHOX_PHOTO.jpg" width="300">
</p>

Akbarshox was responsible for the **coding and software development** of the robot.

### Responsibilities

* Robot programming
* Software development
* Debugging
* Testing robot behavior
* Supporting robot control systems

### Competition Role

Akbarshox performed the **Human Player** role during matches.

---

## 🎨 Azimov Mirsaid

### Designer • Coach

<p align="center">
  <img src="YOUR_MIRSAID_PHOTO.jpg" width="300">
</p>

Mirsaid worked as a **Designer** and **Coach**.

### Responsibilities

* Design-related work
* Team presentation
* Supporting the development process
* Competition preparation

### Competition Role

Mirsaid worked as our **Coach**, supporting communication, strategy, and decision-making during matches.

---

# 🎮 Competition Roles

| Team Member              | Technical Role | Competition Role |
| ------------------------ | -------------- | ---------------- |
| **Shoxruxmirzo Xafizov** | CAD Designer   | 🎮 Driver        |
| **Nazarov Asadbek**      | Engineer       | 🎮 Driver        |
| **Narziyev Akbarshox**   | Coder          | 🕹️ Human Player |
| **Azimov Mirsaid**       | Designer       | 🎧 Coach         |

---

# 🎮 Two-Driver System

Our team used a **two-driver system** during matches.

### Driver 1 — Shoxruxmirzo

Controlled:

* Manipulator mechanisms
* Robot mechanisms
* Mechanism operation during matches

### Driver 2 — Asadbek

Controlled:

* Robot movement
* Driving
* Positioning

The drivers had to communicate and coordinate their actions precisely during matches.

<p align="center">
  <img src="YOUR_DRIVERS_IMAGE.jpg" width="750">
</p>

---

# 🕹️ Human Player

### Akbarshox

<p align="center">
  <img src="YOUR_HUMAN_PLAYER_BADGE.jpg" width="250">
</p>

Akbarshox performed the **Human Player** role during the competition.

His role required coordination with the drivers and the overall match strategy.

---

# 🎧 Coach

### Mirsaid

<p align="center">
  <img src="YOUR_COACH_BADGE.jpg" width="250">
</p>

Mirsaid worked as our **Coach**.

He supported the team through:

* Match strategy
* Communication
* Decision-making
* Team coordination
* Competition planning

---

# 🤖 Our Robot

<p align="center">
  <img src="YOUR_FINAL_ROBOT_IMAGE.jpg" width="800">
</p>

Our robot was developed and prepared specifically for FTC competition.

The development process combined:

* 📐 CAD design
* ⚙️ Mechanical engineering
* 🔌 Electronics
* 💻 Programming
* 🎮 Robot control
* 🧪 Testing
* 🏁 Competition preparation

---

# 📐 CAD & Mechanical Design

CAD was an important part of our robot development.

As the team's CAD Designer, I worked on the mechanical structure and components of the robot.

### CAD Work

* Robot structure
* Mechanical components
* Manipulator mechanisms
* Component positioning
* Design improvements
* Manufacturing preparation

<p align="center">
  <img src="YOUR_CAD_IMAGE_1.jpg" width="750">
</p>

<p align="center">
  <img src="YOUR_CAD_IMAGE_2.jpg" width="750">
</p>

---

# 🔧 Robot Development Process

Our robot went through several stages before reaching its competition-ready form.

## 01 — Planning & Concept

We started by analyzing the competition requirements and discussing possible robot designs and strategies.

<p align="center">
  <img src="YOUR_PROCESS_01.jpg" width="750">
</p>

---

## 02 — CAD Design

We developed the robot structure and mechanisms using CAD.

<p align="center">
  <img src="YOUR_PROCESS_02.jpg" width="750">
</p>

---

## 03 — Building & Assembly

After developing the designs, we built and assembled the robot.

<p align="center">
  <img src="YOUR_PROCESS_03.jpg" width="750">
</p>

---

## 04 — Testing

We tested the robot's mechanisms, movement, and control systems.

<p align="center">
  <img src="YOUR_PROCESS_04.jpg" width="750">
</p>

---

## 05 — Improvements

Testing allowed us to identify problems and improve the robot before the competition.

<p align="center">
  <img src="YOUR_PROCESS_05.jpg" width="750">
</p>

---

## 06 — Final Robot

After multiple iterations, we prepared the final robot for competition.

<p align="center">
  <img src="YOUR_FINAL_ROBOT_IMAGE.jpg" width="800">
</p>

---

# 🧪 Testing & Preparation

Before traveling to Dubai, our team spent time preparing and testing the robot.

We focused on:

* Robot reliability
* Manipulator performance
* Driving practice
* Control systems
* Match strategy
* Team communication
* Troubleshooting

<p align="center">
  <img src="YOUR_PREPARATION_IMAGE.jpg" width="750">
</p>

---

# 🌍 Dubai Competition

<p align="center">
  <img src="YOUR_DUBAI_COMPETITION_IMAGE.jpg" width="750">
</p>

Participating in Dubai gave us the opportunity to test our robot against strong competition and apply everything we had developed during preparation.

The competition environment required us to adapt quickly, communicate effectively, and keep the robot performing reliably throughout the event.

---

# 🏁 Competition Moments

<p align="center">
  <img src="YOUR_MATCH_IMAGE_1.jpg" width="750">
</p>

<p align="center">
  <img src="YOUR_MATCH_IMAGE_2.jpg" width="750">
</p>

<p align="center">
  <img src="YOUR_MATCH_IMAGE_3.jpg" width="750">
</p>

---

# 🏆 Awards & Certificates

## 🥈 2nd Place

<p align="center">
  <img src="YOUR_2ND_PLACE_CERTIFICATE.jpg" width="650">
</p>

Our team finished the competition in **2nd place** as part of the **Finalist Alliance**.

---

## 🏅 Finalist Alliance

<p align="center">
  <img src="YOUR_FINALIST_CERTIFICATE.jpg" width="650">
</p>

Our team reached the **Finalist Alliance**.

---

## 🎖️ Control Award

<p align="center">
  <img src="YOUR_CONTROL_AWARD_CERTIFICATE.jpg" width="650">
</p>

We received the **Control Award** for our robot's control system.

---

# 📸 Team Gallery

## 👥 Full Team

<p align="center">
  <img src="YOUR_TEAM_PHOTO_2.jpg" width="800">
</p>

## 🤖 Team & Robot

<p align="center">
  <img src="YOUR_TEAM_ROBOT_PHOTO.jpg" width="800">
</p>

## 🏆 Awards

<p align="center">
  <img src="YOUR_AWARD_TEAM_PHOTO.jpg" width="800">
</p>

---

# 🎥 Videos

Videos from our preparation and Dubai competition:

* 🎥 **Robot Development:** [VIDEO LINK]
* 🎥 **Practice:** [VIDEO LINK]
* 🎥 **Competition:** [VIDEO LINK]
* 🎥 **Match Footage:** [VIDEO LINK]
* 🎥 **Robot Demonstration:** [VIDEO LINK]

---

# 🧠 Skills Developed

Through this project, our team developed practical experience in:

### Technical Skills

* 🤖 FIRST Tech Challenge
* 📐 CAD Design
* 🔧 Mechanical Engineering
* 💻 Robot Programming
* ⚙️ Mechanism Development
* 🔌 Electronics
* 🎮 Robot Control
* 🧪 Testing & Troubleshooting

### Competition Skills

* 🎮 Driver Operation
* 🕹️ Human Player
* 🎧 Coaching
* 🏁 Match Strategy
* 🤝 Team Communication
* ⚡ Quick Decision Making
* ⏱️ Working Under Pressure

---

# 📚 What We Learned

The Dubai competition taught us how important preparation, reliability, communication, and teamwork are in competitive robotics.

We learned how to take a robot from an initial concept through CAD, construction, programming, testing, and finally competition.

The experience of reaching the **Finalist Alliance**, finishing **2nd place**, and receiving the **Control Award** showed us the importance of combining engineering, software, robot operation, and strategy.

---

# 🚀 Outcome

Our team achieved:

### 🥈 2nd Place

### 🏆 Finalist Alliance

### 🎖️ Control Award

More importantly, the event gave us practical experience in competitive robotics and strengthened our ability to work together as an engineering team.

---

# 👥 Team

**Shoxruxmirzo Xafizov** — CAD Designer / Driver
**Nazarov Asadbek** — Engineer / Driver
**Narziyev Akbarshox** — Coder / Human Player
**Azimov Mirsaid** — Designer / Coach

---

<p align="center">
  <b>🤖 Designed together. Built together. Competed together.</b>
</p>
